...
def create_chart(X, col_names, names,outFile,y_top_lim,labels_post=None):
    if labels_post!=None:
        ones = np.argmax(labels_post >0)
        twos =  np.argmax(labels_post >1)
    ...
    for i,ax in enumerate(axes):
        ...
        x_axis0 = np.arange(ones)
        y_corr0 = X[:ones,xi]
        y_poss0 = X[:ones,xi+1]
        x_axis1 = np.arange(ones,twos)
        y_corr1 = X[ones:twos,xi]
        y_poss1 = X[ones:twos,xi+1]
        x_axis2 = np.arange(twos,len(names))
        y_corr2 = X[twos:,xi]
        y_poss2 = X[twos:,xi+1]


        ax.bar(x_axis0,y_poss0,width,color='#66b3ff')
        ax.bar(x_axis0,y_corr0,width,color='#d62728')
        ax.bar(x_axis1,y_poss1,width,color='#F7DC6F')
        ax.bar(x_axis1,y_corr1,width,color='#d62728')
        ax.bar(x_axis2,y_poss2,width,color='#66ff66')
        ax.bar(x_axis2,y_corr2,width,color='#d62728')
